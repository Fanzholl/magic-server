...
function gatherCoinsOnPath(transport, bounties) {
      bounties.forEach(bounty => {
            const distance = calculateDistance(transport.x, transport.y, bounty.x, bounty.y);
            if (distance <= 200) { // Если монета находится в пределах 200 единиц
                  // Считаем её собранной и корректируем путь
                  transport.x = bounty.x;
                  transport.y = bounty.y;
                  // Здесь можно добавить логику увеличения счёта или бонусов
            }
      });
}
